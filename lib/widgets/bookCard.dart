...
class BookCard extends StatelessWidget {
  ...
  BookCard({@required this.cost,@required this.info,this.action,@required this.category,@required this.color});
  ...
  Widget build(BuildContext context) {
    return  SizedBox(child: Stack(
      children:[Padding(child:Card(color: Colors.white,elevation: 1.0,shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10.0))),
          ...
          children:[
            Align(child: CircleAvatar(backgroundColor: Colors.green[200],maxRadius: 25.0,child: Text("\$${cost??'24.99'}",style: TextStyle(color: Colors.white,fontSize: 10.0))),alignment: Alignment.topRight),
            SizedBox(height: 5.0),
            ...
            Button(color: color??Colors.deepOrange[200], action: action??"buy",textColor: Colors.white)
          ])
